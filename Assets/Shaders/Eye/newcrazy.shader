Shader "Custom/URP/UnlitPsychedelicWithTextures"
{
    Properties
    {
        ...
        _TopTexture ("Top Texture", 2D) = "white" {} // Repeating texture on top
        _TextureScale ("Texture Scale", Range(0.1, 10.0)) = 1.0 // Scale the texture
        _BackgroundTexture ("Background Texture", 2D) = "black" {} // Background texture
        _BackgroundColor ("Background Color", Color) = (0, 0, 0, 1) // Background color
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                // Background texture: Apply the background texture with a color overlay
                float4 backgroundColor = tex2D(_BackgroundTexture, uv) * _BackgroundColor;

                // Apply the background texture on the entire screen (only for the background part)
                finalColor = finalColor + backgroundColor;

                // Repeating texture: Apply the repeating texture on top of the effect
                float2 topTextureUV = uv * _TextureScale;
                float4 topTextureColor = tex2D(_TopTexture, topTextureUV); // Repeating texture

                // Combine top texture with the final color
                finalColor = finalColor + topTextureColor * 0.5;

                return finalColor; // Return the final color with psychedelic effects and textures
            }
